...
import { hlDetail, hlList } from '../shared/hl-animations';
...
export class EticketComponent implements OnInit {
  ...
  retrieveVctRequests(sTime?: Date, eTime?: Date) {
    ...
    if (sTime || eTime) {
      ...
      this.vctService.searchVCTs( this.selectedGroundHandler.key, sUnixTime, eUnixTime )
        .then(
          res => {
      ...
      this.vctService.searchVCTs( this.selectedGroundHandler.key )
        .then(
          res => {
            this.displayVCTs(res);
          }
