...
export class VctService {
  ...
  async searchVCTs(gh: number, sTime?: number, eTime?: number): Promise<Array<ExportVCTRequest>> {
    var params;
    if (sTime) {
    ...
    const response = await this.http.get<string>(this._baseUrl + 'api/vct/' + gh, params ? {params} : {} )
